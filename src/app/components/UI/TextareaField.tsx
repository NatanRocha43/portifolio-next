      ...
      <div className="flex flex-col gap-1.5 w-full">
        ...
        <div className="relative">
          <textarea
            ...
            {...rest}
            className={`w-full px-4 py-3 rounded-xl border bg-white text-[#1A1A1A] placeholder-[#6B7280] text-sm transition-all duration-200 focus:outline-none focus:ring-2 resize-none ${borderClass}`}
          />
          {hasValue && isValid && (
            <CheckCircle2 className="absolute top-3.5 right-3.5 w-4 h-4 text-green-600 pointer-events-none" aria-hidden="true" />
          )}
        </div>
        {error && (
