...
export default function MenuLinks({ onClick, isMobile = false, hidden = false }: MenuLinksProps) {
  ...
  return (
    <ul className={`flex ${isMobile ? "flex-col gap-5 items-center w-full" : "items-center gap-5 lg:gap-7 shrink-0"}`}>
      {links.map(({ label, href }) => (
        <li key={href} className="shrink-0">
          <a
            href={`#${href}`}
            className={`rounded-md px-1 transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-[#7C3AED] focus-visible:outline-none whitespace-nowrap ${
              isMobile
